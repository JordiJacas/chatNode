...
app.get("/", function(req,res){
	res.render("index.html", {title : "Chat con NodeJS, Express, Socket.IO y jQuery"});
});
...
io.sockets.on('connection', function(socket) 
{
	...
	socket.on("loginUser", function(username)
	{
		...
		socket.broadcast.emit("refreshChat", "conectado", socket.username + " se ha conectado");
	...
	socket.on('addNewMessage', function(message) 
	{
		...
		socket.emit("refreshChat", "msg", socket.username + ": " + message);
	});
	...
	socket.on("disconnect", function()
	{
		...
		socket.broadcast.emit("refreshChat", "desconectado",socket.username + " se ha desconectado");
	});
});
